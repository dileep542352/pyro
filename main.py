...
async def account_login(bot: Client, m: Message):
    ...
    try:
        for i in range(arg, len(links)):
            ...
            url = links[i][1]
            name1 = links[i][0].replace("\t", "").replace(":", "").replace("/", "_").replace("+", "").replace("#", "").replace("|", "_").replace("@","").replace("*","").replace(".","").strip()
        ...
        for i in range(arg, len(links)):
            ...
            if "jwplayer" in url:
                headers = {
                    ...
                    'x-access-token': 'ADD CLASS PLUS TOKEN',
                    'user-agent': 'Mobile-Android',
                    'app-version': '1.4.37.1',
                    'api-version': '18',
                    'device-id': ' YOUR DEVICE ID',
                    'device-details': 'DEVICE DETAIL TOO',
